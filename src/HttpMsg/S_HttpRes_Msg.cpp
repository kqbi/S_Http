...
namespace S_Http {
//## class S_HttpRes_Msg
    S_HttpRes_Msg::S_HttpRes_Msg(std::string &connectionId, unsigned status, unsigned version, bool keepAlive,
                                 std::string &body, std::string &contentType, std::uint64_t contentLen) : S_Http_Msg(
            connectionId, body), _keepAlive(keepAlive), _status(status) {
